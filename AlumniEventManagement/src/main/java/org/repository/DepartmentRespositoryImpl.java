...
public class DepartmentRespositoryImpl implements Departmentrepository {
	...
	public List<DepartmentModel> getdept() {
		return template.query("SELECT * FROM department ORDER BY dept_id DESC", new RowMapper<DepartmentModel>() {
			@Override
	...
	public boolean isupdateDepartment(DepartmentModel deptmodel) {
	    String deptName = deptmodel.getDept_name().toUpperCase().trim();
	    ...
	    // If count > 0, the department name already exists
	    if (count > 0) {
	        return false; 
	    }
	    
	    // Perform the update if no existing department found
	    int value = template.update("UPDATE department SET deptname = ? WHERE dept_id = ?", 
	                                deptName, deptmodel.getDept_id());
