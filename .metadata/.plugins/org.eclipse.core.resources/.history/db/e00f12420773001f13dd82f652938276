...
<link
	...
	rel="stylesheet">
<link href="<c:url value='/resources/CSS/updatedept.css' />"
	rel="stylesheet" />
...
<body>
	...
	<div class="content">
		...
		<form name="frm" action="finalupdate" method="POST" class="mt-4"
			onsubmit="return validateDeptName()">
			<div class="row g-3">
				<!-- Department ID Field (Hidden) -->
				<input type="hidden" class="form-control" name="dept_id"
					value="${deptid}" readonly>

				<!-- Department Name Field -->
				<div class="col-md-4">
					<label for="txtDept" class="form-label">Department Name</label> <input
						type="text" class="form-control" name="dept_name" id="txtDept"
						value="${deptname}" placeholder="Enter Department Name"
						autocomplete="off">
